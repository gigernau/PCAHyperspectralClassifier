...
class LiEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, n_planes=2, patch_size=5, pca=5):
        ...
        if(pca > 4 or pca <= 0 ): 
            ...
            self.conv1 = nn.Conv3d(1, n_planes, (pca, 3, 3), padding=(1, 0, 0))    
...
def save_model(model, model_name, dataset_name, **kwargs):
    ...
    if isinstance(model, torch.nn.Module):
        filename = model_name + "_" + dataset_name + "_K={k}".format(
            **kwargs
        )
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            ...
            if patch_size == 1 or center_pixel:
                ...
                output = cp.transpose(output.numpy(), (0, 2, 3, 1))
